...
struct RawResidualJacobian
{
	...
	EIGEN_ALIGN16 VecNRf resF; //r21,这里的r21是对于一个点，八个 pattern residual 组成的向量。

	// the two rows of d[x,y]/d[xi].
	EIGEN_ALIGN16 Vec6f Jpdxi[2]; // 2x6   //∂x2/∂ξ21

	// the two rows of d[x,y]/d[C].
	EIGEN_ALIGN16 VecCf Jpdc[2]; // 2x4  ∂x2/∂C  对相机内参求导

	// the two rows of d[x,y]/d[idepth].
	EIGEN_ALIGN16 Vec2f Jpdd;				// 2x1对逆深度求导

	// the two columns of d[r]/d[x,y].
	EIGEN_ALIGN16 VecNRf JIdx[2];			// 9x2 ∂r21/∂x2

	// = the two columns of d[r] / d[ab]
	EIGEN_ALIGN16 VecNRf JabF[2]; // 9x2  ∂r21/∂a21,∂r21/∂b21
