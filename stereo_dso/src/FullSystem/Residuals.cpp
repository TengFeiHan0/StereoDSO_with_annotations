...
double PointFrameResidual::linearize(CalibHessian* HCalib)
{
	...
	//! @see https://www.cnblogs.com/JingeTU/p/8395046.html
	Vec6f d_xi_x, d_xi_y;
	Vec4f d_C_x, d_C_y;
	float d_d_x, d_d_y;
	{
		float drescale, u, v, new_idepth; //x2'  drescale = ρ2ρ−11
		float Ku, Kv;//x2
		Vec3f KliP;  //KliP = K^−1 x1 = x1'
	...
	for(int idx=0;idx<patternNum;idx++)
	{
		...
		float drdA = (color[idx]-b0);//? 答：对光度参数求导
...

		{
			...
			J->resF[idx] = residual * hw; //r21 是对于一个点，八个 pattern residual 组成的向量//!pattern res Vec(8, 1) resF
			J->JIdx[0][idx] = hitColor[1]; 
			J->JIdx[1][idx] = hitColor[2]; //!bright gradient 
			J->JabF[0][idx] = drdA*hw;
			J->JabF[1][idx] = hw;//!bright a, b

			JIdxJIdx_00 += hitColor[1] * hitColor[1];
			JIdxJIdx_11+=hitColor[2]*hitColor[2];
			JIdxJIdx_10+=hitColor[1]*hitColor[2];

			JabJIdx_00+= drdA*hw * hitColor[1];//方向梯度
	...
	J->JIdx2(0, 0) = JIdxJIdx_00; //（∂r21/∂x2）^T ∂r21/∂x2
	J->JIdx2(0,1) = JIdxJIdx_10;
	J->JIdx2(1,0) = JIdxJIdx_10;
	J->JIdx2(1,1) = JIdxJIdx_11;
	J->JabJIdx(0, 0) = JabJIdx_00; //(∂r21/∂l21)^T ∂r21/∂x2
	J->JabJIdx(0,1) = JabJIdx_01;
	J->JabJIdx(1,0) = JabJIdx_10;
	J->JabJIdx(1,1) = JabJIdx_11;
	J->Jab2(0,0) = JabJab_00; //残差对光度参数a，b的求导
	J->Jab2(0,1) = JabJab_01; //[（∂r21）/∂l21]^T  (∂r21)∂l21
