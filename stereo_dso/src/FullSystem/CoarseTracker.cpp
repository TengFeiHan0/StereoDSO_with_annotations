...
void CoarseTracker::makeCoarseDepthForFirstFrame(FrameHessian* fh)
{
    ...
    for(PointHessian* ph : fh->pointHessians)
    {
        ...
        float weight = sqrtf(1e-3 / (ph->efPoint->HdiF+1e-12));//?权重

        idepth[0][u+w[0]*v] += new_idepth *weight;
        weightSums[0][u+w[0]*v] += weight;//权重之和
        ...
        for(int it=0;it<numIts;it++)
        {
            ...
            for(int i=w[lvl];i<wh;i++)
            {
                if(weightSumsl_bak[i] <= 0)
                {
                    ...
                    if(weightSumsl_bak[i-wl+1] > 0) { sum += idepthl[i-wl+1]; num+=weightSumsl_bak[i-wl+1]; numn++;}
                    if(numn>0) {idepthl[i] = sum/numn; weightSumsl[i] = num/numn;}//深度和权重的平均值
                }
            }
        }
    }
    ...
    for(int lvl=2; lvl<pyrLevelsUsed; lvl++)
    {
        ...
        for(int i=w[lvl];i<wh;i++)
        {
            if(weightSumsl_bak[i] <= 0)
            {
                ...
                if(weightSumsl_bak[i-wl] > 0) { sum += idepthl[i-wl]; num+=weightSumsl_bak[i-wl]; numn++;}
                if(numn>0) {idepthl[i] = sum/numn; weightSumsl[i] = num/numn;}//?为什么要和前两层不一样
            }
        }
    }
	...
	for(FrameHessian* fh : frameHessians)
	{
		for(PointHessian* ph : fh->pointHessians)
		{
			if(ph->lastResiduals[0].first != 0 && ph->lastResiduals[0].second == ResState::IN) //contains information about residuals to the last two (!) frames. ([0] = latest, [1] = the one before).
			{
				...
				// free to debug
                pt_track->idepth_min_stereo = r->centerProjectedTo[2] * 0.1f;//深度
                pt_track->idepth_max_stereo = r->centerProjectedTo[2] * 1.9f;

                ImmaturePointStatus pt_track_right = pt_track->traceStereo(fh_right, K1, 1);//返回右目的帧未成熟点的状态
                ...
                if (pt_track_right == ImmaturePointStatus::IPS_GOOD)
                {
                    ...
                    ImmaturePointStatus pt_track_left = pt_track_back->traceStereo(fh_target, K1, 0);//帧未成熟点的状态

                    float depth = 1.0f/pt_track->idepth_stereo;//之前的双目深度
                    float u_delta = abs(pt_track->u - pt_track_back->lastTraceUV(0));
                    if(u_delta<1 && depth > 0 && depth < 50)//u差值小于1,深度大于0小于五十
                    {
...
void CoarseTracker::calcGSSSE(int lvl, Mat88 &H_out, Vec8 &b_out, SE3 refToNew, AffLight aff_g2l)
{
	...
	__m128 fxl = _mm_set1_ps(fx[lvl]);//_mm_set1_ps对应于_mm_load1_ps的功能，不需要字节对齐，需要多条指令。（r0 = r1 = r2 = r3 = w）
	...
	for(int i=0;i<n;i+=4)
	{
		__m128 dx = _mm_mul_ps(_mm_load_ps(buf_warped_dx+i), fxl);//方向梯度x*fxl
		__m128 dy = _mm_mul_ps(_mm_load_ps(buf_warped_dy+i), fyl);//方向梯度y*fyl
		__m128 u = _mm_load_ps(buf_warped_u+i);
		__m128 v = _mm_load_ps(buf_warped_v+i);
		__m128 id = _mm_load_ps(buf_warped_idepth+i);//逆深度
        //位姿增量的雅可比矩阵，权重，
		acc.updateSSE_eighted(
			_mm_mul_ps(id, dx), //▽Ix ρ2 fx					​
			_mm_mul_ps(id, dy), //▽Iy ρ2 fy
			//-ρ2(▽Ix u2-▽Iy fy v2)
...
Vec6 CoarseTracker::calcRes(int lvl, SE3 refToNew, AffLight aff_g2l, float cutoffTH)
{
  ...
  //	printf("the num of the points is: %d \n", nl);
	for(int i=0;i<nl;i++)
	{
		...
		//Huber weight 残差绝对值是否小于9
        float hw = fabs(residual) < setting_huberTH ? 1 : setting_huberTH / fabs(residual);


		if(fabs(residual) > cutoffTH)//残差大于了这个阈值
		{
	...
	while(numTermsInWarped%4!=0)//余数不为零
	{
	...
	Vec6 rs;
	rs[0] = E;//能量
	rs[1] = numTermsInE;//参与计算E的点的数量
	rs[2] = sumSquaredShiftT/(sumSquaredShiftNum+0.1);//translation only
	rs[3] = 0;
	rs[4] = sumSquaredShiftRT/(sumSquaredShiftNum+0.1);// RT 
	rs[5] = numSaturated / (float)numTermsInE;//大于阈值的数量比上总数量
		...
		IOWrap::Output3DWrapper* wrap)
{
	...
	for(int lvl=coarsestLvl; lvl>=0; lvl--)
	{
        ...
        //一直循环计算直到小于0.6
		while (resOld[5] > 0.6 && levelCutoffRepeat < 50) // numSaturated / (float)numTermsInE大于0.6
		{
			levelCutoffRepeat*=2;
		...
		for(int iteration=0; iteration < maxIterations[lvl]; iteration++)
		{
			...
			incScaled.segment<1>(7) *= SCALE_B;//乘上尺度

            if(!std::isfinite(incScaled.sum())) incScaled.setZero();
			//注意位姿的更新不是李代数相加，而是指数映射到李群后相乘（因为前面求导时用的是扰动模型）
			SE3 refToNew_new = SE3::exp((Vec6)(incScaled.head<6>())) * refToNew_current;//新的位姿
			AffLight aff_g2l_new = aff_g2l_current;//新的光度参数
			aff_g2l_new.a += incScaled[6];
			aff_g2l_new.b += incScaled[7];

			Vec6 resNew = calcRes(lvl, refToNew_new, aff_g2l_new, setting_coarseCutoffTH*levelCutoffRepeat);
			//新的残差小于旧的，可接受
			//   比值 E/numTermsInE	平均能量？
			...
			//如果可以接受，那么就缩小lambda（每次缩小为原来的二分之一），
			//如果优化失败，那么就说明高斯牛顿法的二次函数近似效果在这里不太好，通过增大lambda（放大为原来的4倍）来改善。
			//当某一次得到的增量小于一定值时，认为收敛了，迭代过程终止。
			if(accept)
			{
			...
			//DSO通过比较前后两次误差的大小关系来判断，它要求每一次得到的结果至少要比上一次尝试（上一个候选位姿下的）好，
			//否则就直接跳过；如果比上一次好，那么再看它是否小于设定的阈值，如果小的话就结束尝试。
