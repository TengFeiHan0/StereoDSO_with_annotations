...
void FrameHessian::makeImages(float* color, CalibHessian* HCalib)
{
	...
	{
		dIp[i] = new Eigen::Vector3f[wG[i] * hG[i]]; //dIp 每一层影像的辐射值、x 方向梯度、y 方向梯度
		//absSquaredGrad 存储 xy 方向梯度值的平方和。
	...
	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		for (int idx = wl; idx < wl * (hl - 1); idx++) // 第二行开始
		{
			...
			float dx = 0.5f*(dI_l[idx+1][0] - dI_l[idx-1][0]);//x方向相邻两个的均值
			float dy = 0.5f*(dI_l[idx+wl][0] - dI_l[idx-wl][0]);//y方向的上下两个的均值
