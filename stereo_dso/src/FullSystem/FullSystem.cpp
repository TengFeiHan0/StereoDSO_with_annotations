...
FullSystem::FullSystem()
{
	...
	runMapping=true;
	mappingThread = boost::thread(&FullSystem::mappingLoop, this);//建图线程
	lastRefStopID=0;
...
Vec4 FullSystem::trackNewCoarse(FrameHessian* fh, FrameHessian* fh_right)
{
	...
	{
		FrameShell* slast = allFrameHistory[allFrameHistory.size()-2];//倒数第二帧
		FrameShell* sprelast = allFrameHistory[allFrameHistory.size()-3];//倒数第三帧
		SE3 slast_2_sprelast;//倒数第三相对于第二的位姿
		SE3 lastF_2_slast;//倒数一帧和前一帧的相对位姿
	...
	bool haveOneGood = false;//是不是有一个合适的假设
	int tryIterations=0;//尝试迭代次数
	//对之前所有的假设进行遍历，查找合适的运动假设
	for(unsigned int i=0;i<lastF_2_fh_tries.size();i++)
	{
		...
		bool trackingIsGood = coarseTracker->trackNewestCoarse(fh, lastF_2_fh_this, aff_g2l_this,pyrLevelsUsed-1, achievedRes);	// in each level has to be at least as good as the last try.
		tryIterations++;
		...
		if(haveOneGood)
		{
			for(int i=0;i<5;i++)
			{
				if(!std::isfinite((float)achievedRes[i]) || achievedRes[i] > coarseTracker->lastResiduals[i])	// take over if achievedRes is either bigger or NAN.
					achievedRes[i] = coarseTracker->lastResiduals[i];//选择更低的残差
			}
		}
...
void FullSystem::traceNewCoarseKey(FrameHessian* fh, FrameHessian* fh_right)
{
		...
		for(FrameHessian* host : frameHessians)		// go through all active frames
		{
			...
			for(ImmaturePoint* ph : host->immaturePoints)
			{
				ImmaturePointStatus phTrackStatus = ph->traceOn(fh, KRKi, Kt, aff, &Hcalib, false); //对未成熟点进行分类
...
void FullSystem::activatePointsMT()
{
	...
	FrameHessian* newestHs = frameHessians.back();//最近一帧

	// make dist map.
	coarseDistanceMap->makeK(&Hcalib);
	//将所有的成熟点投影到当前帧，生成距离地图
	...
	for(FrameHessian* host : frameHessians)		// go through all active frames
	{
		...
		for(unsigned int i=0;i<host->immaturePoints.size();i+=1)
		{
			...
			// can activate only if this is true.上一次的投影轨迹长度（极线小于8），次最小误差比最小误差大于3
...
void FullSystem::flagPointsForRemoval()
{
	...
	{
		for(int i=((int)frameHessians.size())-1;i>=0 && i >= ((int)frameHessians.size());i--)//被保留的点
			if(!frameHessians[i]->flaggedForMarginalization) fhsToKeepPoints.push_back(frameHessians[i]);

		for(int i=0; i< (int)frameHessians.size();i++)//需要边缘化的点
 ...
 //@parameters    image            左目图像和曝光参数
  //                image_right      右目
//				  idx
 //@function      程序，添加激活的帧，把所有的加上历史库中。然后建立图像金字塔，判断是否生产关键帧  
 //               
void FullSystem::addActiveFrame( ImageAndExposure* image, ImageAndExposure* image_right, int id )
{

    if(isLost) return;//track线程锁
	boost::unique_lock<boost::mutex> lock(trackMutex);


	//! =========================== add into allFrameHistory =========================
	FrameHessian* fh = new FrameHessian();//FrameHessian 保存影像信息
	FrameHessian *fh_right = new FrameHessian(); //FrameShell 保存帧的位置姿态信息
	FrameShell* shell = new FrameShell();//frameHessians的壳
	shell->camToWorld = SE3(); 		// no lock required, as fh is not used anywhere yet.
	shell->aff_g2l = AffLight(0,0);
    shell->marginalizedAt = shell->id = allFrameHistory.size();
    shell->timestamp = image->timestamp;
	...
	fh->shell = shell;
	fh_right->shell=shell;
	allFrameHistory.push_back(shell);//加入所有历史帧


	//! =========================== make Images / derivatives etc. =========================
	fh->ab_exposure = image->exposure_time;
	//为当前帧的影像建立影像金字塔，并且计算每一层影像的梯度
	fh->makeImages(image->image, &Hcalib);
	fh_right->ab_exposure = image_right->exposure_time;
	fh_right->makeImages(image_right->image,&Hcalib);
	...
	if(!initialized)
	{
		...
		{
			coarseInitializer->setFirstStereo(&Hcalib, fh,fh_right);//初始化
			initialized=true;
		}
		return;
	}
...
void FullSystem::deliverTrackedFrame(FrameHessian* fh, FrameHessian* fh_right, bool needKF)
{
	...
	if(linearizeOperation)
	{
		if(goStepByStep && lastRefStopID != coarseTracker->refFrameID)
		{
			...
			IOWrap::displayImage("frameToTrack", &img);//显示图像
			while(true)
			{
...
void FullSystem::initializeFromInitializer(FrameHessian* newFrame)
{
	...
	firstFrame->idx = frameHessians.size();//赋值给它id
    ...
    frameHessiansRight.push_back(firstFrameRight);//右目的帧

	firstFrame->pointHessians.reserve(wG[0]*hG[0]*0.2f);//20%点的数目
	...
	for(int i=0;i<coarseInitializer->numPoints[0];i++)
	{
        ...
        pt->idepth_min = pt->idepth_min_stereo;//最小深度
        pt->idepth_max = pt->idepth_max_stereo;//最大深度
		...
		ph->setPointStatus(PointHessian::ACTIVE);//活跃点


		firstFrame->pointHessians.push_back(ph);
		ef->insertPoint(ph);//插入活跃点
	}
	    ...
	    for(int x=patternPadding+1;x<wG[0]-patternPadding-2;x++)
	       {
		    ...
		    else newFrame->immaturePoints.push_back(impt);//得到未成熟点
