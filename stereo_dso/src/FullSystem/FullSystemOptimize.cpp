...
void FullSystem::linearizeAll_Reductor(bool fixLinearization, std::vector<PointFrameResidual*>* toRemove, int min, int max, Vec10* stats, int tid)
{
	for(int k=min;k<max;k++)
	{
		...
		(*stats)[0] += r->linearize(&Hcalib);//计算相关导数

		if(fixLinearization)
		{

			//*：一是对于每个 Residual 主要执行 takeDataF()，且更新状态
			//*计算 JpJd ≡ (∂r/∂ξ, ∂r/∂ab)^T * ∂r/∂d
			r->applyRes(true);
...
void FullSystem::setNewFrameEnergyTH()
{
    ...
    newFrame->frameEnergyTH = nthElement*setting_frameEnergyTHFacMedian;//?什么意思
...
float FullSystem::optimize(int mnumOptIts)
{
	...
	Vec3 lastEnergy = linearizeAll(false);//计算导数
	...
	double lambda = 1e-1;
	float stepsize=1;
	VecX previousX = VecX::Constant(CPARS+ 8*frameHessians.size(), NAN);//!4+8×8=68维
	for(int iteration=0;iteration<mnumOptIts;iteration++)
	{
		// solve!
		backupState(iteration != 0);//保存当前所有待优化参数的 state 和 step
			//solveSystemNew(0);
		solveSystem(iteration, lambda);//进行优化（得到的优化变化量 step）
		...
		// eval new energy!
		Vec3 newEnergy = linearizeAll(false); //计算新的能量值
	...
	lastEnergy = linearizeAll(true); //将优化之后成为 outlier 的 residual 剔除
		...
		std::vector<VecX> &nullspaces_affA,std::vector<VecX> &nullspaces_affB)
{
	...
	int n=CPARS+frameHessians.size()*8;//4+8×8
	...
	for(int i=0;i<6;i++)
	{
		...
		for(FrameHessian* fh : frameHessians)
		{
			...
			nullspace_x0.segment<3>(CPARS+fh->idx*8) *= SCALE_XI_TRANS_INVERSE;//平移
			nullspace_x0.segment<3>(CPARS+fh->idx*8+3) *= SCALE_XI_ROT_INVERSE;//旋转
		}
	...
	for(int i=0;i<2;i++)
	{
		...
		for(FrameHessian* fh : frameHessians)
		{
			...
			nullspace_x0[CPARS+fh->idx*8+6] *= SCALE_A_INVERSE;//a
			nullspace_x0[CPARS+fh->idx*8+7] *= SCALE_B_INVERSE;//b
		}
		nullspaces_x0_pre.push_back(nullspace_x0);
