...
CoarseInitializer::CoarseInitializer(int ww, int hh) : thisToNext_aff(0,0), thisToNext(SE3())
{
	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		points[lvl] = 0;
		numPoints[lvl] = 0;
		idepth[lvl] = new float[wl*hl];//初始化
	}
	...
	printDebug=false;
    //?什么意思  是各个变量的尺度信息
	wM.diagonal()[0] = wM.diagonal()[1] = wM.diagonal()[2] = SCALE_XI_ROT;//旋转
	wM.diagonal()[3] = wM.diagonal()[4] = wM.diagonal()[5] = SCALE_XI_TRANS;//平移
	wM.diagonal()[6] = SCALE_A;//光度参数a，b
...
{
	newFrame = newFrameHessian;//?为什么newFrameHessian_Right没有被操作
	...
	SE3 refToNew_current = thisToNext;//相对位姿
	AffLight refToNew_aff_current = thisToNext_aff;//光度参数
	...
	for(int lvl=pyrLevelsUsed-1; lvl>=0; lvl--)
	{
		...
		if(lvl<pyrLevelsUsed-1)
			propagateDown(lvl + 1); //将当前层所有点的逆深度设置为的它们 parent （上一层）的逆深度
		...
		{
			Mat88f Hl = H;
		    for(int i=0;i<8;i++) Hl(i,i) *= (1+lambda);// GN优化 
			// 舒尔补, 边缘化掉逆深度状态
			...
			if(fixAffine)
			{
				...
				inc.tail<2>().setZero();//光度参数a,b设置为0
			}
			else
				inc = - (wM * (Hl.ldlt().solve(bl)));	//=-H^-1 * b.

             //先存入局部变量中，不确定是否接受这一步的优化结果
			...
			if(accept)
			{
				...
				applyStep(lvl);//保存优化结果
				optReg(lvl);   //将所有点的 iR 设置为其 neighbour 逆深度的中位数
				lambda *= 0.5;
				fails=0;
	...
	for(int i=0;i<pyrLevelsUsed-1;i++)
		//使用低一层点的逆深度更新其高一层点 parent 的逆深度，这个更新是基于 iR 的，使得逆深度平滑
			propagateUp(i);
		...
		bool plot)
{
	...
	for(int i=0;i<npts;i++)
	{
		...
		EIGEN_ALIGN16 VecNRf dp6;  //残差/a2
		EIGEN_ALIGN16 VecNRf dp7;   // 残差/b2
		EIGEN_ALIGN16 VecNRf dd; // 残差/逆深度
		...
		for(int idx=0;idx<patternNum;idx++)
		{
			...
			//!see https://www.cnblogs.com/JingeTU/p/8203606.html 残差推导
			if(hw < 1)
				hw = sqrtf(hw);	////?? 为啥开根号, 答: 鲁棒核函数等价于加权最小二乘
			float dxInterp = hw*hitColor[1]*fxl;//hw*gx*fx
			float dyInterp = hw*hitColor[2]*fyl;//hw*gy*fy
			dp0[idx] = new_idepth * dxInterp;   //rou2*gx*fx
			dp1[idx] = new_idepth * dyInterp;   //rou2*gy*fy
		...
		//!@see https://www.cnblogs.com/JingeTU/p/8297076.html
		// 内点则加进能量函数
		// add into energy.
		E.updateSingle(energy);
		point->isGood_new = true;
		point->energy_new[0] = energy;
		//! 一次操作取 4 个 float，完成 4 个不同数据、相同命令的 float 运算。
		//这种一次 4 个数据的操作，说明了循环的 i 增量是 4。
		//算法现在使用的 patternNum 是 8（8%4==0），所以当前循环可以完成所有 pattern 点的操作。
		//如果 patternNum%4 != 0，剩下的余数就可以用第二个循环把最后两个 bit 设置为 0
		//得到了小于 patternNum 的最大的 4 的倍数，这个循环每次进行 1 个float运算，把最后未循环到的 pattern 点补足。
	...
	for(int i=0;i<npts;i++)
	{
		...
		{
			point->energy_new[1] = (point->idepth_new-1)*(point->idepth_new-1);//? 为什么是这么计算
	...
	//!@see https://www.cnblogs.com/JingeTU/p/8297076.html
	acc9SC.initialize();//schur complement
	for(int i=0;i<npts;i++)
	{
		Pnt* point = ptsl+i;
		if(!point->isGood_new)
		...
		//分母里多了一个1，猜测是为了防止JbBuffer_new[i][9]太小造成系统不稳定。
		JbBuffer_new[i][9] = 1 / (1 + JbBuffer_new[i][9]); // 对应的是1/JρJTρ
	...
	b_out = acc9.H.topRightCorner<8, 1>();   // / acc9.num;//!dp^T*dp
	...
	void CoarseInitializer::propagateDown(int srcLvl)
{
	...
	for(int i=0;i<nptst;i++)
	{
		...
		else
		{
			...
			// iR可以看做是逆深度的值, 使用的高斯归一化积, Hessian是信息矩阵
...
void CoarseInitializer::setFirstStereo(	CalibHessian* HCalib, FrameHessian* newFrameHessian, FrameHessian* newFrameHessian_Right)
{
	...
	bool* statusMapB = new bool[w[0]*h[0]];//像素选择
	...
	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		sel.currentPotential = 3;//设置网格大小 3×3
		int npts,npts_right;//选择的像素数目
		if(lvl == 0)//第0层提取特征像素
		{
		...
		if(points[lvl] != 0) delete[] points[lvl];//删除之前的非空点
		points[lvl] = new Pnt[npts];//创建新的 good points

		// set idepth map by static stereo matching. if no idepth is available, set 0.01.
		int wl = w[lvl], hl = h[lvl];//每一层图像大小
		    ...
		    for(int x=patternPadding+1;x<wl-patternPadding-2;x++)
		       {
				   ...
				   if (lvl == 0 && statusMap[x + y * wl] != 0)
				   {
					   ...
					   if (stat == ImmaturePointStatus::IPS_GOOD)
					   {
						   ...
						   //outlier的阈值与pattern的大小有关, 一个像素是12*12
...
void CoarseInitializer::doStep(int lvl, float lambda, Vec8f inc)
{
	...
	for(int i=0;i<npts;i++)
	{
		...
		//对于每一个点，代码中inc对应δx21，JbBuffer[i].head<8>() 对应Hρx21的第 8 * i 行到第 8 * i + 7 行，
		//JbBuffer[i][8] 对应JTρr21的第 8 * i 行到第8 *i + 7 行，
		//JbBuffer[i][9] 对应H−1ρρ的(i, i)位置上的 Scalar。
		//! dd*r + (dp*dd)^T*delta_p
		float b = JbBuffer[i][8] + JbBuffer[i].head<8>().dot(inc); //H−1ρρ(J^Tρ*r21+Hρx21*δx21)

		//! dd * delta_d = dd*r - (dp*dd)^T*delta_p = b
		//! delta_d = b * dd^-1
		float step = -b * JbBuffer[i][9] / (1 + lambda); //δρ

		float maxstep = maxPixelStep*pts[i].maxstep;
		if(maxstep > idMaxStep) maxstep=idMaxStep;

		if(step >  maxstep) step = maxstep;//限制范围
		if(step < -maxstep) step = -maxstep;
        //更新后的逆深度
		float newIdepth = pts[i].idepth + step;
		if(newIdepth < 1e-3 ) newIdepth = 1e-3;
		if(newIdepth > 50) newIdepth = 50;
		pts[i].idepth_new = newIdepth;//得到逆深度
	}
