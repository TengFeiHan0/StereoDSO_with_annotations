...
void FullSystem::flagFramesForMarginalization(FrameHessian* newFH)
{
	...
	for(int i=0;i<(int)frameHessians.size();i++)
	{
		...
		//当活跃的帧的数量大于最低要求（５个）时，边缘化一帧中活跃的点少于５ % 或者和最新的帧相比光度参数变化剧烈（∣a1−a2∣> 0.7 的帧（从最早的帧开始遍历）
...
//					fh->statistics_tracesCreatedForThisFrame, fh->statistics_pointsActivatedForThisFrame);
           fh->flaggedForMarginalization = true;
            //!Frames with less than 5 % of their points visible in I1 are marginalized.
            flagged++;
		}
		else
		{
...
//					fh->statistics_tracesCreatedForThisFrame, fh->statistics_pointsActivatedForThisFrame);
	...
	if((int)frameHessians.size()-flagged >= setting_maxFrames)
	{
		...
		for(FrameHessian* fh : frameHessians)
		{
			...
			distScore *= -sqrtf(fh->targetPrecalc.back().distanceLL);//得到距离分数
