...
*@ function: 		
*@ param:    hist           直方图
*@           below          setting_minGradHistCut=0.5

*@ note                     占据 below% 的梯度值作为阈值
*@return                    阈值
*/
...
*@note                     生成梯度直方图, 为每个block计算阈值（平滑前和平滑后的）
*/
		...
		float* map_out, float density, int recursionsLeft, bool plot, float thFactor)  //thFactor = settingImaturePointDensity
{
   ...
   /*	if(setting_pixelSelectionUseFast>0 && allowFast)
	...
	else*/
	{
		...
		if(fh != gradHistFrame) makeHists(fh);//生成梯度直方图
		...
		quotia = numWant / numHave;
		//计算新的采像素点的, 范围大小, 相当于动态网格了, pot越小取得点越多
		...
		//想要的数目和已经得到的数目, 大于或小于0 .25都会重新采样一次 
		...
		float* map_out, int pot, float thFactor)
{
	...
	// 金字塔层阈值的减小倍数
	float dw1 = setting_gradDownweightPerLevel; //0.75// 第二层
	float dw2 = dw1*dw1;//0.5625//第三层
