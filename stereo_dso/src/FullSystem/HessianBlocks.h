...
struct FrameHessian;//FrameHessian即是一个带着状态变量与Hessian信息的帧。
struct PointHessian;//所有活跃点的信息

class ImmaturePoint;//未成熟的点
class FrameShell;
...
struct FrameHessian
{
	...
	int frameID;//关键帧id						// incremental ID for keyframes only!
	static int instanceCounter; //!< 计数器
	int idx;

	// Photometric Calibration Stuff
	float frameEnergyTH;	// 能量阈值，set dynamically depending on tracking residual
	float ab_exposure;

	bool flaggedForMarginalization;//边缘化的flag
